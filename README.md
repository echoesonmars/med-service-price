...
Архитектура проекта разделена на три автономные части, обеспечивающие масштабируемость, отказоустойчивость и высокую скорость работы.
...
*   **`/backend`** — Основное API проекта. Быстрая обработка запросов пользователей, полнотекстовый и семантический поиск по каноническому словарю услуг, работа с геопозицией (выбор города) и интерактивная диагностика симптомов через ИИ.
...
### 2. Этап поиска и выдачи (Реалтайм, `/backend`)
...
*   **Умный путь (поиск по симптомам)**: Если пользователь вводит симптомы (например, *"болит бок"*), бэкенд через быстрый промпт LLM переводит запрос в список рекомендуемых услуг, а затем ищет их цены в базе данных.
