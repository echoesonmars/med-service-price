...
/** Return a random integer in [min, max] rounded to nearest 500 */
...
// ─── Clinic-to-Category mapping (which categories each clinic offers) ───────

interface ClinicProfile {
  namePattern: string; // partial match on clinic name
  categories: string[];
...
function getCategoriesForClinic(clinicName: string): string[] {
  // Dental clinics
  if (
    clinicName.includes('Стоматолог') ||
    clinicName.includes('White Smile') ||
    clinicName.includes('Тіс')
  ) {
    return ['стоматология', 'диагностика'];
  }
  // Lab-only clinics
  if (
    clinicName.includes('Лаборатория') ||
    clinicName.includes('InVitro') ||
    clinicName.includes('KDL') ||
    clinicName.includes('Олекс')
  ) {
...
function getServiceCountRange(clinicName: string): [number, number] {
  ...
  if (clinicName.includes('Многопрофильная') || clinicName.includes('МедЭлит') || clinicName.includes('Президент')) {
    return [40, 60];
  }
  return [25, 45];
}
